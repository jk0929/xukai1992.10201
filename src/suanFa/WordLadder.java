...
public class WordLadder {
    ...
    private static boolean oneCharOff( String word1, String word2 ) {
            if( word1.length( ) != word2.length( ) )//判断单词的长度是否相同；
                return false;

            int diffs = 0;

            for( int i = 0; i < word1.length( ); i++ )//判断两个单词中有几个字母不一样，如果字母不同数目超过1，则返回false;
    ...
    {
            Map<String,List<String>> adjWords = new TreeMap<>( );//TreeMap是sortMap的实现类；
            Map<Integer,List<String>> wordsByLength = new TreeMap<>( );

              // Group the words by their length
            for( String w : words )//根据单词的长度，进行分组，并组装到map中，key为长度,list为指定长度的单词；
                update( wordsByLength, w.length( ), w );
